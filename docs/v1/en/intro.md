...
Harness framework for <span className="hs-hero__accent">distributed, enterprise-grade</span> agents.
</h1>
...
<div className="hs-window__tabs">
 <button type="button" aria-pressed={true} className="hs-tab active" data-panel="en-harness">HarnessAgent</button> <button type="button" aria-pressed={false} className="hs-tab" data-panel="en-react">ReActAgent</button>
</div>

</div>

<div className="hs-code-panel" id="en-react" style={{"display": "none"}}>
...
var agent = ReActAgent.builder()
    ...
    .build();
// Autonomous reasoning + tool calling
...
<div className="hs-install">
 <code>io.agentscope:agentscope-harness</code> <button className="hs-copy-btn" data-copy-base64="aW8uYWdlbnRzY29wZTphZ2VudHNjb3BlLWhhcm5lc3M=">Copy Maven</button>
</div>
...
<div className="hs-stat">
 <span className="hs-stat__val">JDK 17+</span> <span className="hs-stat__label">minimum Java version</span>
</div>

<div className="hs-stat">
 <span className="hs-stat__val">Reactive</span> <span className="hs-stat__label">built on Project Reactor</span>
</div>

<div className="hs-stat">
 <span className="hs-stat__val">MCP · A2A</span> <span className="hs-stat__label">open protocol support</span>
</div>

<div className="hs-stat">
 <span className="hs-stat__val">Apache 2.0</span> <span className="hs-stat__label">open-source license</span>
</div>
...
Local disk → Docker sandbox → object storage, one line to switch
 ...
 <a href="/v1/en/docs/harness/overview" className="hs-btn hs-btn--secondary" style={{"marginTop": "4px"}}>Learn about Harness →</a>
</div>
...
Sub-agents can independently inherit or override Harness config
 ...
 <a href="/v1/en/docs/harness/subagent" className="hs-btn hs-btn--secondary" style={{"marginTop": "4px"}}>Learn about multi-agent →</a>
</div>
...
<a className="hs-card" href="/v1/en/docs/quickstart/agent">
...
Autonomous planning, tool calling, and result integration. Built-in safe interruption, graceful cancellation, and hook-based human-in-the-loop oversight.
 ...
 <span className="hs-card__link">Quick start →</span>
</a>

<a className="hs-card" href="/v1/en/docs/harness/memory">
...
<a className="hs-card" href="/v1/en/docs/task/mcp">
...
Connect to any MCP-compatible server — file systems, databases, browsers, code interpreters — instantly extending agent tool capabilities without custom integration code.
 ...
 <span className="hs-card__link">View MCP →</span>
</a>

<a className="hs-card" href="/v1/en/docs/task/a2a">
...
Register agents with Nacos or similar service registries. Other agents can discover and delegate tasks as naturally as calling a microservice.
 ...
 <span className="hs-card__link">View A2A →</span>
</a>

<a className="hs-card" href="/v1/en/docs/harness/sandbox/index">
...
Tool execution in isolated environments (local Unix, Docker, E2B). Sandbox state is fully preserved across multi-turn conversations with cross-session snapshot restore.
 ...
 <span className="hs-card__link">Learn about sandboxes →</span>
</a>

<a className="hs-card" href="/v1/en/docs/task/observability">
...
Pluggable Tracer SPI with native OpenTelemetry support for distributed tracing. AgentScope Studio provides visual debugging and real-time monitoring dashboards.
 ...
 <span className="hs-card__link">View Studio →</span>
</a>
...
Run your first agent in minutes. Start with a basic ReActAgent, then layer in Harness engineering, multi-agent orchestration, and enterprise capabilities as you need them — the same API scales with you.
 ...
 <a href="/v1/en/docs/quickstart/installation" className="hs-btn hs-btn--primary">Start building →</a>
</div>
...
<p>
Still have questions? Ask on <a href="https://github.com/agentscope-ai/agentscope-java/discussions" style={{"color": "var(--hs-accent)"}}>GitHub Discussions</a>.
</p>
...
<p>
AgentScope Java requires <code>JDK 17</code> or higher. The framework uses modern Java features such as Records and Sealed Classes, and is built on Project Reactor for a non-blocking reactive execution model. For ultra-low cold-start latency, native image compilation via Quarkus and GraalVM is supported.
</p>
...
<p>
Any model compatible with the OpenAI Chat Completions API is supported out of the box, including Alibaba Cloud Qwen, OpenAI GPT series, Anthropic Claude, and local Ollama instances. The framework provides a standard <code>ModelConfig</code> SPI for adding custom model providers via plugins.
</p>
...
<p>
A <code>ReActAgent</code> is the core reasoning–tool–response loop. <code>HarnessAgent</code> injects workspace loading, memory persistence, context compaction, and sandbox lifecycle management on top of the same reasoning core via the Hook system. You can start with <code>ReActAgent</code> and migrate seamlessly to <code>HarnessAgent</code> when you need production engineering capabilities — no business logic changes required.
</p>

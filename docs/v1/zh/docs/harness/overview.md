...
**扩展方向**：在工作区里放 `KNOWLEDGE.md`、`skills/*/SKILL.md`、`subagents/*.md` 就能分别开启领域知识注入、技能加载、子 agent 编排；`.toolResultEviction(ToolResultEvictionConfig.defaults())` 一行启用大结果卸载；**文件/命令的落点**用 [Filesystem — 三种声明式模式](/v1/zh/docs/harness/filesystem#三种声明式模式) 选择 **共享存储、沙箱或本机+shell**；需隔离执行时优先 `filesystem(SandboxFilesystemSpec)`（见 [Sandbox](/v1/zh/docs/harness/sandbox/index)），`abstractFilesystem` 仅作自管后端的逃生口。
...
- **可插拔文件系统** —— 解决 *agent 的环境如何收敛与隔离*。所有文件工具都走 `AbstractFilesystem`；通过 [三种声明式模式](/v1/zh/docs/harness/filesystem#三种声明式模式)（本机+shell、复合+Store、沙箱）或 `abstractFilesystem` 自管；配合 `RuntimeContext.userId` 与 `IsolationScope` 做多租户/会话级隔离。隔离执行与沙箱状态恢复见 [Sandbox](/v1/zh/docs/harness/sandbox/index)。
...
- **Hook 通道**：按 `priority` 把若干 hook 交给 `ReActAgent`（含沙箱模式下的 `SandboxLifecycleHook` 等，详见 [Architecture](/v1/zh/docs/harness/architecture)）
...
> 各组件的详细行为、触发时机和时序图见 [Architecture](/v1/zh/docs/harness/architecture)。

## 延伸阅读

- [Architecture](/v1/zh/docs/harness/architecture) — 各组件定义、生命周期时序图、协作关系
- [工作区（Workspace）](/v1/zh/docs/harness/workspace) — 工作区目录结构与上下文注入
- [记忆（Memory）](/v1/zh/docs/harness/memory) — 双层记忆、对话压缩与全文检索
- [文件系统（Filesystem）](/v1/zh/docs/harness/filesystem) — 三种声明式模式与 `AbstractFilesystem` 层次
- [沙箱（Sandbox）](/v1/zh/docs/harness/sandbox/index) — 隔离执行、沙箱状态与分布式选项
- [子 Agent（Subagent）](/v1/zh/docs/harness/subagent) — 子 agent 规格与编排
- [子 Agent 流式输出](/v1/zh/docs/harness/streaming) — `stream()` 模式下子 agent 事件转发、`EventSource` 字段与多级嵌套（通用流式基础见 [task/streaming](/v1/zh/docs/task/streaming)）
- [工具（Tool）](/v1/zh/docs/harness/tool) — 内置工具参考
- [技能（Skill）](/v1/zh/docs/harness/skill) — 一键接入 skill 市场、工作区共享与按用户隔离的四层合成
- [会话（Session）](/v1/zh/docs/harness/session) — 会话持久化与状态恢复
